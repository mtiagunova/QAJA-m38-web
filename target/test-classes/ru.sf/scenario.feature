Feature:
    # 1. Позитивный сценарий: открыть страницу бесплатных мероприятий  с помощью кнопки
    # 'Бесплатные мероприятия' на главной странице
  Scenario: open free events page with 'free events' button
    ...
    # 2. Позитивный сценарий: открыть страницу 'ВСЕ КУРСЫ' с помощью выпадающего списка Курсы
    #  и перейти на курс 'Data Scientist'
  ...
  Scenario: course difference info closes when click button after opening
    ...
    # 10. Позитивный сценарий: открыть страницу 'Бесплатные мероприятия' по кнопке 'Бесплатно' в шапке сайта
  Scenario: open free events page with 'Free' navigation bar button
    Given url of service 'https://skillfactory.ru/'
    Then click free nav bar button
    And assert you are on 'https://skillfactory.ru/events' page

    # 11. Позитивный сценарий: открыть страницу 'Курсы по Data Science' по кнопке 'Data Science' на главной странице
